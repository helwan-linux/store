...
static void backend_report_output(
    const char *output)
{
...
static void strip_ansi_sequences(
    char *text)
{
    if (!text)
        return;
    ...
    int current_percent)
{
    ...
    if (strstr(
        ...
        strstr(text, "Retrieving packages") != NULL ||
...
/* =========================================================
 * Count available system updates
 * ========================================================= */

int backend_get_update_count(void)
{
    if (!handle)
        return 0;
...
gboolean backend_system_has_updates(void)
{
    return (
        backend_get_update_count() > 0
    );
}
