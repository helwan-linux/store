    ...
    GString *scan_buffer)
{
    ...
    /* تتبع الكلمات المفتاحية لمراحل باكمان لتحديث العداد بسلاسة */
    if (strstr(scan_buffer->str, "resolving dependencies") != NULL) {
        latest_percent = 10;
    } else if (strstr(scan_buffer->str, "looking for conflicting packages") != NULL) {
        latest_percent = 20;
    } else if (strstr(scan_buffer->str, "Downloading") != NULL) {
        latest_percent = 40;
    } else if (strstr(scan_buffer->str, "checking keyring") != NULL || strstr(scan_buffer->str, "checking package integrity") != NULL) {
        latest_percent = 60;
    } else if (strstr(scan_buffer->str, "loading package files") != NULL) {
        latest_percent = 75;
    } else if (strstr(scan_buffer->str, "installing") != NULL || strstr(scan_buffer->str, "removing") != NULL) {
        latest_percent = 90;
    }
...
/* =========================================================
 * Remove (تم التعديل لاستخدام -Rs لحذف الحزمة والتبعيات بنجاح)
 * ========================================================= */
