...
/* =========================================================
 * Modern CSS Styling (External File Loading)
 * ========================================================= */
...
static void apply_custom_stylesheet(void) {
    ...
    gtk_css_provider_load_from_path(provider, "data/style.css", &error);

    if (error != NULL) {
        g_warning("Could not load CSS file: %s", error->message);
        g_error_free(error);
    ...
    char *argv[])
{
    ...
    // تحميل الستايل من الملف الخارجي
